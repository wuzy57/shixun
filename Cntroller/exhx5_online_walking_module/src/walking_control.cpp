...
#include <stdio.h>
#include "exhx5_online_walking_module/walking_control.h"
...
void WalkingControl::calcFootStepParam()
{
  ...
  for (int i=0; i<foot_step_size_; i++)
  {
    ...
    /*if (foot_step_command_.command == "left_turning" && foot_step_command_.start_leg == "left_leg")
    {
      if (i == 0 ||
          i == 1 ||
          i == foot_step_size_-1)
      ...
      if (i == (foot_step_size_-1))
      {
        ...
        if ((foot_step_size_-1) % 2 !=0 )
        {
          ROS_WARN("Turn Right, please check whether the step number is even or not!");
        }
      }
    }
    else if (foot_step_command_.command == "right_turning" && foot_step_command_.start_leg == "right_leg")
    {
      if (i == 0 ||
          i == 1 ||
          i == foot_step_size_-1)
      ...
      if (i == (foot_step_size_-1))
      {
        ...
        if ((foot_step_size_-1) % 2 !=0 )
        {
          ROS_WARN("Turn Left, please check whether the step number is even or not!");
        }
      }      
    }
    else
    ...
    if (i == foot_step_size_-2 || i == foot_step_size_-1)    // will return to init pose at the end when do not add i == foot_step_size_-2
    {
      msg.x = 0;
      msg.y = foot_origin_shift_y_;
      theta = 0.0;
      ...
      if (foot_step_command_.command == "left_turning" && foot_step_command_.start_leg == "left_leg") 
      {
        ...
        if ((foot_step_size_-1) % 2 !=0 )
        {
          ROS_WARN("Turn Right instead of Turn Left, please check whether the step number is even or not!");
        }
      ...
      if (foot_step_command_.command == "right_turning" && foot_step_command_.start_leg == "right_leg") 
      {
        msg.x = -foot_step_command_.step_length;
        if ((foot_step_size_-1) % 2 !=0 )
        {
          ROS_WARN("Turn Left instead of Turn Left, please check whether the step number is even or not!");
        }
        if (i == foot_step_size_-2)
        {
          msg.x = foot_step_command_.step_length; 
