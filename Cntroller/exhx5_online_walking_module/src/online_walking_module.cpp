...
#include "exhx5_online_walking_module/online_walking_module.h"
    ...
    foot_distance_(0.147)
{
  ...
  exhx5_kdl_ = new EXHX5Kinematics();
  ...
  std::string balance_gain_path = ros::package::getPath("exhx5_online_walking_module") + "/config/balance_gain.yaml";
  parseBalanceGainData(balance_gain_path);

  std::string joint_feedback_gain_path = ros::package::getPath("exhx5_online_walking_module") + "/config/joint_feedback_gain.yaml";
  parseJointFeedbackGainData(joint_feedback_gain_path);

  std::string joint_feedforward_gain_path = ros::package::getPath("exhx5_online_walking_module") + "/config/joint_feedforward_gain.yaml";
...
void OnlineWalkingModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  // Service
//  get_preview_matrix_client_ = ros_node.serviceClient<exhx5_online_walking_module_msgs::GetPreviewMatrix>("/robotis/online_walking/get_preview_matrix", 0);
}
...
void OnlineWalkingModule::calcRobotPose()
{
  ...
  // Forward Kinematics
  exhx5_kdl_->initialize(des_body_pos, des_body_rot);
  ...
  exhx5_kdl_->setJointPosition(r_leg_joint_pos, l_leg_joint_pos);
  ...
  exhx5_kdl_->solveForwardKinematics(r_leg_pos, r_leg_Q,
                                   l_leg_pos, l_leg_Q);
...
bool OnlineWalkingModule::setBalanceControl()
{
  ...
  // ======= ======= //
  exhx5_kdl_->initialize(des_body_pos_mod, des_body_rot_mod);
  ...
  ik_success = exhx5_kdl_->solveInverseKinematics(r_leg_output,
